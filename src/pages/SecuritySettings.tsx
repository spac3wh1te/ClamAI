  ...
  return (
    <div>
      <div className="px-4 py-2.5 cursor-pointer hover:bg-secondary/30 flex items-center gap-3" onClick={() => setOpen(!open)}>
        ...
        <span className={`text-[10px] px-1.5 py-0.5 rounded ${cfg.bg || "bg-secondary"} ${cfg.color || "text-muted-foreground"} font-medium`}>{record.risk_level || "unknown"}</span>
...
function AdvancedPolicy() {
  ...
  const [form, setForm] = useState<SystemAnalysisConfig>({
    enabled: true, model: "", api_key_id: "", time_range: "7d",
    interval_minutes: 60, notify_on_high_risk: true, auto_block_risk_level: "", system_prompt: "",
  });

  useEffect(() => {
    if (config) {
      setForm({
        enabled: config.enabled, model: config.model, api_key_id: "",
        time_range: config.time_range, interval_minutes: config.interval_minutes,
        notify_on_high_risk: config.notify_on_high_risk, auto_block_risk_level: config.auto_block_risk_level,
  ...
  const showMsg = (text: string, type: "ok" | "err") => { setMsg({ text, type }); setTimeout(() => setMsg(null), 3000); };

  const updateMutation = useMutation({
    mutationFn: (cfg: SystemAnalysisConfig) => systemAnalysisApi.updateConfig(cfg),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["system-analysis-config"] }); queryClient.invalidateQueries({ queryKey: ["system-analysis-tasks"] }); showMsg("配置已保存", "ok"); },
  ...
  const handleTrigger = useCallback(async () => {
    ...
    try {
      ...
      if ((resp as any).success === false) { showMsg((resp as any).message || "触发失败", "err"); } else { showMsg("已触发执行", "ok"); setTimeout(() => refetchHistory(), 3000); }
    } catch (e: any) { showMsg(e?.message || "触发失败", "err"); } finally { setTimeout(() => setTriggering(false), 3000); }
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="p-4 space-y-4">
          <div className="flex items-center gap-3 p-3 bg-emerald-500/5 border border-emerald-500/20 rounded-lg">
            <div className="flex items-center gap-2">
              {form.enabled ? (
                <><div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" /><span className="text-sm text-emerald-400 font-medium">已启用</span></>
              ) : (
                <><div className="w-2 h-2 rounded-full bg-muted-foreground" /><span className="text-sm text-muted-foreground">已禁用</span></>
              )}
            </div>
            <span className="text-xs text-muted-foreground ml-2">开启后系统将按照配置的间隔自动执行行为分析任务</span>
            <button onClick={() => { const newEnabled = !form.enabled; setForm({ ...form, enabled: newEnabled }); updateMutation.mutate({ ...form, enabled: newEnabled }); }}
              className={`ml-auto px-3 py-1.5 text-xs rounded-md border transition-colors ${form.enabled ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/30" : "bg-secondary text-muted-foreground border-border"}`}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select value={form.model} onChange={(e) => setForm({ ...form, model: e.target.value })} className="w-full px-3 py-2 bg-background border border-border rounded-lg text-sm">
            ...
            <div>
              ...
              <div className="px-3 py-2 bg-secondary border border-border rounded-lg text-sm text-muted-foreground">自动分析所有 API Key</div>
            </div>
            <div>
              <label className="text-xs text-muted-foreground font-medium block mb-1.5">分析时间范围</label>
              <select value={form.time_range} onChange={(e) => setForm({ ...form, time_range: e.target.value })} className="w-full px-3 py-2 bg-background border border-border rounded-lg text-sm">
            ...
            <div>
              ...
              <select value={form.interval_minutes} onChange={(e) => setForm({ ...form, interval_minutes: parseInt(e.target.value) })} className="w-full px-3 py-2 bg-background border border-border rounded-lg text-sm">
          ...
          <div className="flex items-center gap-4 pt-2">
            <label className="flex items-center gap-2 text-sm cursor-pointer">
              <input type="checkbox" checked={form.notify_on_high_risk} onChange={(e) => setForm({ ...form, notify_on_high_risk: e.target.checked })} className="w-4 h-4 rounded border-border" />
          ...
          <div className="flex items-center gap-3 pt-2">
            <button onClick={() => updateMutation.mutate(form)} disabled={updateMutation.isPending}
              ...
              <Save size={14} />{updateMutation.isPending ? "保存中..." : "保存配置"}
            </button>
            <button onClick={handleTrigger} disabled={triggering || isRunning}
              className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm transition-colors ${triggering || isRunning ? "bg-secondary text-muted-foreground cursor-not-allowed" : "bg-amber-500/10 text-amber-400 border border-amber-500/30 hover:bg-amber-500/20"}`}>
              {triggering || isRunning ? <Loader2 size={14} className="animate-spin" /> : <Play size={14} />}
              {triggering || isRunning ? "执行中..." : "立即执行一次"}
            </button>
            {msg && <span className={`text-xs px-2 py-1 rounded ${msg.type === "ok" ? "text-emerald-400 bg-emerald-500/10" : "text-red-400 bg-red-500/10"}`}>{msg.text}</span>}
          </div>

          <details className="mt-3">
            <summary className="text-xs text-muted-foreground cursor-pointer hover:text-foreground select-none">分析提示词配置</summary>
            <div className="mt-3 space-y-3">
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={async () => { try { const resp = await systemAnalysisApi.getDefaultPrompt(); const newForm = { ...form, system_prompt: resp.prompt }; setForm(newForm); updateMutation.mutate(newForm); } catch {} }}
      ...
      {systemTask && (
        <div className="bg-card rounded-xl border border-border">
          <div className="px-4 py-3 border-b border-border flex items-center gap-2">
            <Clock size={14} className="text-muted-foreground" /><span className="text-sm font-semibold">当前任务状态</span>
          </div>
          <div className="p-4">
            <div className="flex items-center gap-4">
              ...
              {systemTask.next_run_at && systemTask.schedule_type === "periodic" && <span className="text-xs text-muted-foreground">下次: {formatTime(systemTask.next_run_at)}</span>}
            </div>
            {systemTask.result_summary && <p className="text-sm text-muted-foreground mt-2">{systemTask.result_summary}</p>}
          </div>
        </div>
      )}
      ...
      {history.length > 0 && (
        <div className="bg-card rounded-xl border border-border">
          <div className="px-4 py-3 border-b border-border flex items-center gap-2">
            <Clock size={14} className="text-muted-foreground" /><span className="text-sm font-semibold">历史分析记录</span>
            <span className="text-xs text-muted-foreground ml-2">{history.length} 次</span>
            <button onClick={() => refetchHistory()} className="ml-auto p-1 text-muted-foreground hover:text-foreground"><RefreshCw size={14} /></button>
          </div>
          <div className="divide-y divide-border max-h-96 overflow-y-auto">
            {history.map((record) => {
              ...
              if (record.detail) { try { const p = JSON.parse(record.detail); if (Array.isArray(p)) keyResults = p; } catch {} }
              return <HistoryRecord key={record.id} record={record} cfg={cfg} formatTime={formatTime} keyResults={keyResults} />;
            })}
          </div>
        </div>
      )}
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">内容安全策略、检测规则与威胁分析配置</p>
      </div>
      <div className="flex gap-2">
        {TABS.map((t) => (
          <button key={t.id} onClick={() => setTab(t.id)}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${tab === t.id ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground hover:bg-secondary/80"}`}>
