...
impl ServiceManager {
        ...
        match self.config_manager.try_lock() {
            Ok(mgr) => {
                ...
                match config.deploy_mode {
                    DeployMode::PC => format!("http://localhost:{}", config.admin_port),
                    DeployMode::Server => {
                        config
                            .remote_service_url
                            .unwrap_or_else(|| format!("http://localhost:{}", config.admin_port))
                    }
                }
            }
    ...
    pub fn get_proxy_url(&self) -> String {
        match self.config_manager.try_lock() {
            Ok(mgr) => {
                let config = mgr.get_config();
                match config.deploy_mode {
                    DeployMode::PC => format!("http://localhost:{}", config.port),
                    DeployMode::Server => {
                        config
                            .remote_proxy_url
                            .or(config.remote_service_url)
    ...
    pub async fn start_proxy_service(&mut self) -> Result<()> {
        ...
        match config.deploy_mode {
            ...
            DeployMode::Server => {
                if config.remote_service_url.is_none() {
                    return Err(ClamAIError::ProxyService(
                        "Server模式下未配置远程服务地址".to_string(),
                    ));
                }
                ...
                tracing::info!(
                    "[ServiceManager] Server模式：检查远程服务 {}",
                    check_url
                );
                ...
                match timeout(Duration::from_secs(10), client.get(&check_url).send()).await {
                    ...
                    Ok(Ok(resp)) => {
                        ...
                        Err(ClamAIError::ProxyService(format!(
                            "远程服务返回状态码: {}",
                            status
                        )))
                    }
                    Ok(Err(e)) => Err(ClamAIError::ProxyService(format!(
                        "连接远程服务失败: {}",
                        e
                    ))),
                    Err(_) => {
    ...
    async fn start_local_service(
        ...
        tracing::info!(
            "[ServiceManager] 获取网关配置: port={}, admin_port={}, host={}",
            config.port,
            config.admin_port,
            config.host
        );

        {
            ...
            if process_guard.is_some() {
                tracing::warn!(
                    "[ServiceManager] 代理服务已在运行中，拒绝重复启动"
                );
        ...
        let original_admin = config.admin_port;

        let actual_port = Self::find_available_port(config.port, "模型服务").await;
        let actual_admin = Self::find_available_port(config.admin_port, "管理").await;

        let port_changed = actual_port != original_port || actual_admin != original_admin;
        if port_changed {
            tracing::warn!(
            ...
            cfg.admin_port = actual_admin;
            mgr.update_config(cfg)
                .await
                .map_err(|e| ClamAIError::ProxyService(format!("保存端口配置失败: {}", e)))?;
        }
        ...
        tracing::info!(
            "[ServiceManager] Go代理进程已启动，获取到child"
        );
        ...
        *process_guard = Some(child);
        ...
        let service_url = format!("http://127.0.0.1:{}", actual_admin);
        let health_url = format!("{}/health", service_url);
        tracing::info!(
            "[ServiceManager] 等待代理服务健康检查: {}",
            health_url
        );
        ...
        for attempt in 1..=30 {
            match timeout(Duration::from_secs(2), client.get(&health_url).send()).await {
                ...
                Ok(Ok(resp)) => {
                    tracing::debug!(
                        "[ServiceManager] 健康检查返回状态: {} (attempt {})",
                        resp.status(),
                        attempt
                    );
                }
                Ok(Err(e)) => {
                    tracing::debug!(
                        "[ServiceManager] 健康检查连接失败: {} (attempt {})",
                        e,
                        attempt
                    );
                }
        ...
        if !healthy {
            tracing::warn!(
                "[ServiceManager] 代理服务健康检查未通过，但进程已启动，继续执行"
            );
        }
        ...
        tracing::info!(
            "✅ 本地代理服务启动成功，模型端口: {}, 管理端口: {}",
            start_config.port,
            start_config.admin_port
        );
        if port_changed {
            tracing::info!(
                "⚠️ 端口已自动调整(原 模型:{}, 管理:{})",
    ...
    async fn find_available_port(start: u16, label: &str) -> u16 {
        ...
        tracing::error!(
            "[ServiceManager] 100个端口范围内均无法找到可用的{}端口",
            label
        );
        start
    }
    ...
    pub fn get_deploy_mode(&self) -> DeployMode {
        match self.config_manager.try_lock() {
            Ok(mgr) => mgr.get_config().deploy_mode,
            Err(_) => DeployMode::PC,
        }
    }
    ...
    pub async fn get_service_status(&self) -> ServiceStatus {
        ...
        let is_running = match config.deploy_mode {
        ...
        let service_url = match config.deploy_mode {
            DeployMode::PC => format!("http://localhost:{}", config.admin_port),
            DeployMode::Server => config.remote_service_url.unwrap_or_default(),
        };
        ...
        ServiceStatus {
            ...
            deploy_mode: match config.deploy_mode {
